...
class CodeStyleConfig:
    def __init__(
        ...
        if indent_type is not None and indent_type not in [
            INDENT_TYPE_SPACE,
            INDENT_TYPE_TAB,
        ]:
            raise CodeStyleConfigInitError("indent_type must be 'space' or 'tab'")

        if indent_width is not None and indent_width < 0:
            raise CodeStyleConfigInitError("indent_width must be a positive integer")
        ...
        if template_file is not None and not os.path.exists(template_file):
            raise CodeStyleConfigInitError(
                "The specified template file '{}' is not found".format(template_file)
            )
        if indent_type is not None:
            self.indent_type = indent_type
        elif (
            lang.default_code_style is not None
            and lang.default_code_style.indent_type is not None
        ):
        ...
        if indent_width is not None:
            ...
            lang.default_code_style is not None
            and lang.default_code_style.indent_width is not None
        ):
        ...
        if code_generator_toml is not None:
            ...
            try:
                module = imm.SourceFileLoader(
                    "code_generator", code_generator_file
                ).load_module()
        ...
        self.template_file = normalize_path(template_file or lang.default_template_path)

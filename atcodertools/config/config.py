...
def get_config_dic(config_dic, config_type: ConfigType, lang=None):
    ...
    if config_type == ConfigType.TESTER:
        ...
        result = _update_config_dict(result, {"compile_command": compile_command})
...
class Config:
    ...
    def load(
        cls,
        fp: TextIO,
        get_config_type: Set[ConfigType],
        ...
        if ConfigType.TESTER in get_config_type:
            ...
            if args:
                tester_config_dic = _update_config_dict(
                    tester_config_dic,
                    dict(
        ...
        if ConfigType.SUBMIT in get_config_type:
            submit_config_dic = get_config_dic(config_dic, ConfigType.SUBMIT, lang)
            if args:
                submit_config_dic = _update_config_dict(
                    submit_config_dic,
                    dict(
                        exec_before_submit=args.exec_before_submit,
        ...
        if ConfigType.COMPILER in get_config_type:
            ...
            if args:
                compiler_config_dic = _update_config_dict(
                    compiler_config_dic,
                    dict(
                        compile_only_when_diff_detected=args.compile_only_when_diff_detected,
                        compile_command=args.compile_command,

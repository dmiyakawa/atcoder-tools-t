...
PROB_URL_RE = re.compile(r'"/contests/.*/tasks/([A-Za-z0-9\'~+\-_]+)"')
SUBMISSION_URL_RE = re.compile(r"/submissions/([0-9]+)")
...
class Submission:
    ...
    def make_submissions_from(html: str):
        ...
        return [
            Submission(pid, int(sid))
            for pid, sid in zip(submitted_problem_ids, submission_ids)
        ]

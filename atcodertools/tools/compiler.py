...
def _compile(
    ...
    if not force_compile:
        ...
        if (
            os.path.exists(exec_path_name)
            and code_path.stat().st_mtime < pathlib.Path(exec_path_name).stat().st_mtime
        ):
...
def compile_main_and_judge_programs(
    lang: Language, cwd="./", force_compile=False, compile_command=None
) -> None:
    print("[Main Program]")
...
def main(prog, args):
    ...
    parser.add_argument(
        ...
        help="compile only when diff detected [true, false]" " [Default]: false",
        type=bool,
        default=None,
    )
    ...
    compile_main_and_judge_programs(
        metadata.lang, force_compile=force_compile, compile_command=compile_command
    )

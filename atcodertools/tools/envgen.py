...
def prepare_procedure(atcoder_client: AtCoderClient, problem: Problem, config: Config):
    ...
    problem_dir_path = os.path.join(
        workspace_root_path, problem.get_contest().get_id(), pid
    )
    ...
    code_file_path = os.path.join(problem_dir_path, "main.{}".format(lang.extension))
    ...
    if os.path.exists(code_file_path):
        ...
        while True:
            ...
            backup_id += 1
        emit_info(
            "Backup for existing code '{}' -> '{}'".format(code_file_path, new_path)
        )
    ...
    create_code(
        code_generator(
            CodeGenArgs(
                template, prediction_result.format, constants, config.code_style_config
            )
        ),
        code_file_path,
    )
    ...
    if config.postprocess_config.exec_cmd_on_problem_dir is not None:
        emit_info(
            _message_on_execution(
                problem_dir_path, config.postprocess_config.exec_cmd_on_problem_dir
            )
        )
...
def prepare_contest(
    ...
    if config.postprocess_config.exec_cmd_on_contest_dir is not None:
        ...
        logger.info(
            _message_on_execution(
                contest_dir_path, config.postprocess_config.exec_cmd_on_contest_dir
            )
        )
...
def get_config(args: argparse.Namespace) -> Config:
    def _load(path: str) -> Config:
        ...
        with open(path, "r") as f:
            return Config.load(
                f, {ConfigType.CODESTYLE, ConfigType.POSTPROCESS, ConfigType.ETC}, args
            )
...
def main(prog, args):
    ...
    parser.add_argument(
        "--without-login", action="store_true", help="Download data without login"
    )
            ...
            "\n".join(
                [
                    "[Default ({dname})] {path}".format(
                        dname=lang.display_name, path=lang.default_template_path
                    )

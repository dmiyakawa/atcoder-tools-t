...
def main(prog, args) -> None:
    ...
    output_metadata = Metadata.load_from(os.path.join(args.dir, "metadata.json"))
    ...
    if (
        new_metadata_judge_type is not None
        and new_metadata_judge_type != old_metadata_judge_type
    ):
        if new_metadata_judge_type == JudgeType.Normal.value:
            ...
            if args.error_value is None:
                logger.warn("Error-value is not specified. DEFAULT_EPS is set")
                output_metadata.judge_method.diff = DEFAULT_EPS
        else:
            raise NoJudgeTypeException()

    if (
        new_metadata_judge_type == JudgeType.Decimal.value
        and args.error_value is not None
    ):

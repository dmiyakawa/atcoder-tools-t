...
from atcodertools.tools.compiler import (
...
class TestSummary:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.has_error_output == other.has_error_output
        )
...
def infer_exec_file(filenames: List[str], excluded_exec_files: List[str]):
    exec_files = [
        ...
        if is_executable_file(name) and (name not in excluded_exec_files)
    ]
...
def build_details_str(exec_res: ExecResult, input_file: str, output_file: str) -> str:
    ...
    if exec_res.status != ExecStatus.NORMAL:
        append(
            with_color(
                "Aborted ({})\n".format(exec_res.status.name), Fore.LIGHTYELLOW_EX
            )
        )
...
def run_for_samples(
    ...
    for in_sample_file, out_sample_file in sample_pair_list:
        # Run program
        exec_res = run_program(
            exec_file, in_sample_file, timeout_sec=timeout_sec, current_working_dir=cwd
        )
...
def run_single_test(
    ...
    judge_method: Judge,
    cwd: str,
    judge_program_language: Language,
) -> bool:
...
def run_all_tests(
    ...
    if len(in_sample_file_list) != len(out_sample_file_list):
        ...
        raise IrregularSampleFileError
    samples = []
    for in_sample_file, out_sample_file in zip(
        in_sample_file_list, out_sample_file_list
    ):
        validate_sample_pair(in_sample_file, out_sample_file)
    ...
    if len(samples) == 0:
        ...
        print(
            "{msg} (passed {success_count} of {total})".format(
        ...
        print(
            with_color(
                "Passed all test case but with stderr. (Please remove stderr!)",
                Fore.LIGHTYELLOW_EX,
            )
        )
        return False
...
def get_metadata(metadata_file: str) -> Metadata:
    try:
        ...
        logger.warning(
            "{} is not found. Default metadata is selected. ".format(metadata_file)
        )
...
def _decide_judge_method(
    args: argparse.Namespace, metadata: Metadata, lang: Optional[Language]
):
    def _decide_decimal_judge():
        ...
        if args.judge_type:
            assert args.judge_type in ["absolute", "relative", "absolute_or_relative"]
    ...
    if args.judge_type is not None:
        if args.judge_type == "normal":
            ...
            logger.error(
                "Unknown judge type: {}. judge type must be one of [{}]".format(
...
def main(prog, args) -> bool:
    ...
    parser.add_argument(
        "--error-value",
        "-v",
        help="error value for decimal number judge:" " [Default] " + str(DEFAULT_EPS),
        type=float,
        default=None,
    )
    ...
    parser.add_argument(
        ...
        help="compile source before testing: " " [Default]: None",
        action="store_true",
        default=None,
    )

    parser.add_argument(
        "--compile-only-when-diff-detected",
        help="compile only when diff detected" " [Default]: None",
        action="store_true",
        default=None,
    )
    ...
    parser.add_argument(
        "--compile-command",
        help="set compile command" " [Default]: None",
        type=str,
        default=None,
    )
    ...
    if isinstance(judge_method, DecimalJudge):
        logger.info(
            "Decimal number judge is enabled. type={}, diff={}".format(

...
def main(
    prog, args, credential_supplier=None, use_local_session_cache=True, client=None
) -> bool:
    ...
    parser.add_argument(
        "--exec-before-submit",
        help="exec command before submit:" " [Default] None",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "--exec-after-submit",
        help="run command after submit:" " [Default] None",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "--submit-filename",
        help="file for submit will changed to this name:" " [Default] None",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "--lang",
        help=(
            "Use the specified language setting instead of what is set in your config file."
    ...
    try:
        ...
        logger.error(
            "{0} is not found! You need {0} to use this submission functionality.".format(
                metadata_file
            )
        )
        return False
    ...
    if args.lang:
        ...
        logger.info(
            f"Choosing {lang.name} for the language instead of {metadata.lang.name}"
        )
    else:
        lang = metadata.lang
    ...
    if client is None:
        try:
            ...
            client.login(
                save_session_cache=not args.save_no_session_cache,
    ...
    if args.force or tester.main("", tester_args):
        ...
        for encoding in ["utf8", "utf-8_sig", "cp932"]:
            try:
                with open(
                    os.path.join(args.dir, code_path), "r", encoding=encoding
                ) as f:
                    source = f.read()
                break
        ...
        logger.info(
            "{} {}".format(with_color("Done!", Fore.LIGHTGREEN_EX), submissions_url)
        )

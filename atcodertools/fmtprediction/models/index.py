...
class Index:
    ...
    def get_length(self):
        ...
        return CalcNode.parse(
            "{max_index}-({min_index})+1".format(
                max_index=self.max_index, min_index=self.min_index
            )
        ).simplify()
    ...
    def _update_min(self, new_value: str):
        if not new_value.isdecimal():
            ...
            return
        if (self.min_index is None) or (
            self.min_index.evaluate() > CalcNode.parse(new_value).evaluate()
        ):
    ...
    def _update_max(self, new_value: str):
        ...
        if (self.max_index is None) or (
            len(self.max_index.get_all_variables()) == 0
            and self.max_index.evaluate() < CalcNode.parse(new_value).evaluate()
        ):
            self.max_index = CalcNode.parse(new_value)
